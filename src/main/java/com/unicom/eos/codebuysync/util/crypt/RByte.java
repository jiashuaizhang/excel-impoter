...
public class RByte {
    ...
    private static final Logger LOG = LoggerFactory.getLogger(RByte.class);
    ...
    public static long toLong(byte[] bytes, int offset, final int length) {
        if (length != SIZEOF_LONG || offset + length > bytes.length) {
            throw explainWrongLengthOrOffset(bytes, offset,
                    length, SIZEOF_LONG);
        }
        long l = 0;
    ...
    public static byte[] toBytes(final float f) {
        // Encode it as int
        return RByte.toBytes(Float.floatToRawIntBits(f));
    }
    ...
    public static byte[] toBytes(final double d) {
        // Encode it as a long
        return RByte.toBytes(Double.doubleToRawLongBits(d));
    }
     ...
     * @return
     */
    public static byte[] trimTail(byte[] bytes, byte trimByte) {
        int length = bytes.length;
        for (; length > 0; length--) {
            if (bytes[length - 1] != trimByte) {
                break;
            }
        }
    ...
    public static byte[] subBytes(byte[] bytes, int fromIndex) {
        int length = bytes.length - fromIndex;
        return subBytes(bytes, fromIndex, length);
    }

    public static byte[] subBytes(byte[] bytes, int fromIndex, int length) {
        byte[] ret = new byte[length];
        System.arraycopy(bytes, fromIndex, ret, 0, length);
